    ...
    <div class="userinfo">
        <div class="user fl">
            <router-link tag="div" class="userimg fl" :to="{name: 'userInfo'}">
                ...
                <img src="./static/img/userimg.png" v-else>
            </router-link>
            <div class="username font34 font_blod color1470cc fl">
                ...
                <router-link tag="span" class="username" v-else :to="{name: 'login'}">请登录</router-link>
            </div>
        </div>
        <router-link tag="div" class="shopbtn fr font26 colorff9500 bgfff tc" v-if="userInfo.username" :to="(isSetShop ? {name: 'shopInfo'} : {name: 'registerShop'})">{{isSetShop ? "进入店铺" : "开设店铺"}}</router-link>
    </div>
</template>
